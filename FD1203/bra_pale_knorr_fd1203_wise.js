...
var files = fs.readdirSync("C:/Pulse/FD12-03_LOGS/"); //Leer documentos
...
function idle(){
  ...
  for (var k=0;k<files.length;k++){//Verificar los archivos
    var stats = fs.statSync("C:/Pulse/FD12-03_LOGS/"+files[k]);
    ...
    try{
          ...
          client1.on('connect', function(err) {
            ...
            client1.readHoldingRegisters(0, 16).then(function(resp) {
                FillerctOut=( joinWord(resp.register[0], resp.register[1]) + joinWord(resp.register[2], resp.register[3]) + joinWord(resp.register[4], resp.register[5]) + joinWord(resp.register[6], resp.register[7]));
                CartonerctIn=( joinWord(resp.register[0], resp.register[1]) + joinWord(resp.register[2], resp.register[3]) + joinWord(resp.register[4], resp.register[5]) + joinWord(resp.register[6], resp.register[7]));
            ...
            //------------------------------------------Filler----------------------------------------------
              ...
              if (FillerflagPrint == 1) {
                for (var key in Fillerresults) {
                  ...
                  fs.appendFileSync('C:/Pulse/FD12-03_LOGS/bra_pou_Filler_FD12-03.log', 'tt=' + Fillertime + ',var=' + key + ',val=' + Fillerresults[key] + '\n');
                }
                FillerflagPrint = 0;
              }
            ...
            //------------------------------------------Cartoner----------------------------------------------
              ...
              if (CartonerflagPrint == 1) {
                for (var key in Cartonerresults) {
                  ...
                  fs.appendFileSync('C:/Pulse/FD12-03_LOGS/bra_pou_Cartoner_FD12-03.log', 'tt=' + Cartonertime + ',var=' + key + ',val=' + Cartonerresults[key] + '\n');
                }
                CartonerflagPrint = 0;
              }
            ...
            //////--Encoder--------------------------------------------------------------------------------------------------------------------
              ...
              if (EncoderflagPrint == 1) {
                for (var key in Encoderresults) {
                  fs.appendFileSync('C:/Pulse/FD12-03_LOGS/bra_pou_Encoder_FD12-03.log', 'tt=' + Encodertime + ',var=' + key + ',val=' + Encoderresults[key] + '\n');
                }
                EncoderflagPrint = 0;
              }
            //////--Encoder--------------------------------------------------------------------------------------------------------------------
        //------------------------------------------FillerDisplay----------------------------------------------
              ...
              if (FillerDisplayflagPrint == 1) {
                for (var key in FillerDisplayresults) {
                  ...
                  fs.appendFileSync('C:/Pulse/FD12-03_LOGS/bra_pou_FillerDisplay_FD12-03.log', 'tt=' + FillerDisplaytime + ',var=' + key + ',val=' + FillerDisplayresults[key] + '\n');
                }
          ...
          //------------------------------------------FillerDisplay----------------------------------------------
            ...
            //////--CasePacker--------------------------------------------------------------------------------------------------------------------
              ...
              if (CasePackerflagPrint == 1) {
                for (var key in CasePackerresults) {
                  ...
                  fs.appendFileSync('C:/Pulse/FD12-03_LOGS/bra_pou_CasePacker_FD12-03.log', 'tt=' + CasePackertime + ',var=' + key + ',val=' + CasePackerresults[key] + '\n');
                }
            ...
            //////--CasePacker--------------------------------------------------------------------------------------------------------------------
               ...
               //////--EOL--------------------------------------------------------------------------------------------------------------------
            if(secEOL>=60){
              fs.appendFileSync("C:/Pulse/FD12-03_LOGS/bra_pou_EOL_FD12-03.log","tt="+Date.now()+",var=EOL"+",val="+eol+"\n");
